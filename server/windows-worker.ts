...
$explorers = @(Get-CimInstance Win32_Process -Filter "Name = 'explorer.exe'" -ErrorAction SilentlyContinue | ForEach-Object {
  $process = $_
  ...
  if ($owner.ReturnValue -eq 0) {
    ...
    if ($ownerName -ieq $currentUser) { [pscustomobject]@{ SessionId = [int]$process.SessionId } }
  }
})
